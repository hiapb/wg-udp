...
FEC_WORKDIR="/etc/udpspeeder"
FEC_PSK_FILE="${FEC_WORKDIR}/psk"
FEC_CLIENT_PORT_FILE="${FEC_WORKDIR}/client_port"
FEC_SERVER_PORT_FILE="${FEC_WORKDIR}/server_port"
FEC_REMOTE_FILE="${FEC_WORKDIR}/remote"

# 精准剥离冗余开销后的黄金 MTU 阈值
WG_SAFE_MTU=1400
FEC_DEFAULT_PORT=40008
...
setup_exit_services() {
  ...
  mkdir -p "$FEC_WORKDIR"
  echo "$psk" > "$FEC_PSK_FILE"
  echo "$listen_port" > "$FEC_SERVER_PORT_FILE"
...
[Service]
Type=simple
ExecStart=${UDPSPEEDER_BIN} -s -l 0.0.0.0:${listen_port} -r 127.0.0.1:${wg_port} -f20:10 -k "${psk}" --mode 0 -q1
Restart=on-failure
User=root
...
EOF
  ...
  systemctl enable udpspeeder-exit.service >/dev/null 2>&1 || true
  systemctl restart udpspeeder-exit.service || true

  echo "✅ FEC链路服务端 (UDPspeeder) 已配置并尝试启动"
}

setup_entry_services() {
  local remote_ip="$1"
  local remote_port="$2"      
  local speeder_local="39999" 
  local psk="$3"

  mkdir -p "$FEC_WORKDIR"
  echo "$psk" > "$FEC_PSK_FILE"
  echo "$speeder_local" > "$FEC_CLIENT_PORT_FILE"
  echo "${remote_ip}:${remote_port}" > "$FEC_REMOTE_FILE"
...
[Service]
Type=simple
ExecStart=${UDPSPEEDER_BIN} -c -l 127.0.0.1:${speeder_local} -r ${remote_ip}:${remote_port} -f20:10 -k "${psk}" --mode 0 -q1
Restart=on-failure
User=root
...
EOF
  ...
  systemctl daemon-reload
  systemctl enable udpspeeder-entry.service >/dev/null 2>&1 || true
  systemctl restart udpspeeder-entry.service || true

  echo "✅ FEC链路客户端 (UDPspeeder) 已配置并尝试启动"
}
...
configure_exit() {
  ...
  local FEC_PORT
  read -rp "外层隧道监听端口 (默认 ${FEC_DEFAULT_PORT}): " FEC_PORT
  FEC_PORT=${FEC_PORT:-$FEC_DEFAULT_PORT}
  if [[ "$FEC_PORT" == "89" ]]; then
  ...
  local DEFAULT_PSK
  if [[ -f "$FEC_PSK_FILE" ]]; then
    DEFAULT_PSK=$(cat "$FEC_PSK_FILE")
  else
    DEFAULT_PSK=$(head -c 16 /dev/urandom | base64)
  fi
  read -rp "链路预共享密钥 PSK (默认自动生成): " FEC_PSK
  FEC_PSK=${FEC_PSK:-$DEFAULT_PSK}
...
EOF
  ...
  setup_exit_services 51820 "$FEC_PORT" "$FEC_PSK"

  echo
  echo "====== 综合链路连接信息（给入口服务器用）======"
  echo "出口公网 IP：${EXIT_PUBLIC_IP}"
  echo "远端监听端口：${FEC_PORT}"
  echo "PSK：${FEC_PSK}"
  echo "=============================================="
  echo
  echo "出口服务器配置完成，当前状态："
...
enable_global_mode() {
  ...
  local FEC_REMOTE_PORT="$FEC_DEFAULT_PORT"
  if [[ -f "$FEC_REMOTE_FILE" ]]; then
    local remote_str
    remote_str=$(cat "$FEC_REMOTE_FILE" 2>/dev/null || true)
    if [[ "$remote_str" == *:* ]]; then
      FEC_REMOTE_PORT="${remote_str##*:}"
    fi
  fi
  ...
  # 放行底层传输端口
  iptables -t mangle -C OUTPUT -p tcp --dport "${FEC_REMOTE_PORT}" -j RETURN 2>/dev/null || \
    iptables -t mangle -A OUTPUT -p tcp --dport "${FEC_REMOTE_PORT}" -j RETURN
  iptables -t mangle -C OUTPUT -p udp --dport "${FEC_REMOTE_PORT}" -j RETURN 2>/dev/null || \
    iptables -t mangle -A OUTPUT -p udp --dport "${FEC_REMOTE_PORT}" -j RETURN
...
configure_entry() {
  ...
  read -rp "出口服务器隧道监听端口 (默认 ${FEC_DEFAULT_PORT}): " FEC_REMOTE_PORT
  FEC_REMOTE_PORT=${FEC_REMOTE_PORT:-$FEC_DEFAULT_PORT}
  ...
  read -rp "请输入与出口服务器一致的 PSK: " FEC_PSK
  FEC_PSK=${FEC_PSK:-$DEFAULT_PSK}

  setup_entry_services "$EXIT_PUBLIC_IP" "$FEC_REMOTE_PORT" "$FEC_PSK"
...
uninstall_wg() {
  ...
  if [[ "$confirm" =~ ^[yY]$ ]]; then
    ...
    # 强制进行卸载前置状态回滚：切换至分流模式以清理全局路由表和 NAT 残留规则
    if [[ "$(get_role 2>/dev/null)" == "entry" ]]; then
      echo "[*] 正在执行状态回滚：强制切换至分流模式以剥离内核路由钩子..."
    ...
    systemctl stop udpspeeder-exit.service udpspeeder-entry.service 2>/dev/null || true
    systemctl disable udpspeeder-exit.service udpspeeder-entry.service 2>/dev/null || true
    rm -f /etc/systemd/system/udpspeeder-*.service 2>/dev/null || true
    systemctl daemon-reload || true

    rm -rf "$FEC_WORKDIR" /etc/wireguard 2>/dev/null || true
    rm -f "$UDPSPEEDER_BIN" 2>/dev/null || true
...
update_entry_remote_ip() {
  local unit="/etc/systemd/system/udpspeeder-entry.service"
  [[ -f "$unit" ]] || return 1
  read -rp "新出口公网 IP: " new_ip
  sed -i -E "s/( -r )[[:graph:]]+:[0-9]+/\1${new_ip}:$(cat $FEC_REMOTE_FILE | cut -d: -f2)/" "$unit"
  systemctl daemon-reload
  systemctl restart udpspeeder-entry.service
  echo "✅ IP 已更新为 $new_ip"
}
